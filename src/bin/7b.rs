...
fn parse_line(line: &str) -> (u64, Vec<u64>) {
    ...
    let operands = parts.next().unwrap().split(" ").map(|x| x.parse().unwrap()).collect();
    (result, operands)
}
        ...
        .fold(operands[0], |acc, (operand, operator)| {
            match operator {
                ...
                Operator::Concatenate => {
                    format!("{}{}", acc, operand).parse().unwrap()
                }
            }
        })
}
...
fn find_valid_operator_combination(operands: &[u64], target: u64) -> Option<Vec<Operator>> {
    ...
    let mut operators_combinations = (0..num_operators)
        .map(|_| [Operator::Add, Operator::Multiply, Operator::Concatenate])
        .multi_cartesian_product();

    operators_combinations.find(|operators| {
        evaluate_term(operands, operators.as_ref()) == target
    }).clone()
}
...
fn main() {
    ...
    let sum_valid_results = expressions.into_iter().filter_map(|(result, operands)| {
        find_valid_operator_combination(&operands, result).map(|_| {
            result
        })
    }).sum::<u64>();
